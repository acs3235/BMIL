...
if ~isempty(a)
    i=1;
    while ~isempty(dir([filename num2str(i) '.mci']))
        i=i+1;
    end
    input_filename=[filename num2str(i) '.mci'];
    output_filename=[filename num2str(i) '.mco'];
    
    disp(['Filename ' filename '.mci already in use. Using ' input_filename ' instead.']);
else
