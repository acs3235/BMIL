...
function h=create_FLMC_MCR_input_file(  filename,...            % Name of the created input (.mci) and output (.mco) files used to communicate with MCML
                                        number_of_photons,...   % Number of photon packets to simulate
...
if ~isempty(a)
    i=1;
    while ~isempty(dir([filename num2str(i) '.inr']))
        i=i+1;
    end
    input_filename=[filename num2str(i) '.inr'];
    output_filename=[filename num2str(i) '.mcr'];
    
    disp(['Filename ' filename '.ina already in use. Using ' input_filename ' instead.']);
else
    input_filename=[filename '.inr'];
    output_filename=[filename '.mcr'];
end
...
end %function create_FLMC_MCR_input_file