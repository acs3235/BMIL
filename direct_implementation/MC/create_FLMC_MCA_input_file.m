...
% 
% Useage: 
%   h=create_FLMC_input_file(filename, AorB, number_of_photons, layers, n_above, n_below, dz, dr, number_of_dz, number_of_dr, number_of_da);
%
% Arguments:
...
%                                   unique name and avioding overwriting existing simulation results.
% 	AorB,...                      - Defines the output file format binary (B) or ASCII (A)
...
%	number_of_da);                - (optional, default value 1) Number of grid elements, a-direction. 
%
% Output:
...
% Example
%
% >> h=create_MCML_input_file('simulation','A',50000, [1.4 0.1 10 0.8 2]);
...
%
%
% >> h=create_MCML_input_file('simulation2','A',50000, [1.4 0.1 10 0.8 2],1,2,0.1);
...
%
%
% >> h=create_MCML_input_file('simulation_two_layers','A',50000, [1.4 0.1 10 0.8 2;1.4 0.2 10 0.8 100]);
...
function h=create_FLMC_MCA_input_file(  filename,...            % Name of the created input (.mci) and output (.mco) files used to communicate with MCML
                                        number_of_photons,...   % Number of photon packets to simulate
...
if ~isempty(a)
    i=1;
    while ~isempty(dir([filename num2str(i) '.ina']))
        i=i+1;
    end
    input_filename=[filename num2str(i) '.ina'];
    output_filename=[filename num2str(i) '.mca'];
    
    disp(['Filename ' filename '.ina already in use. Using ' input_filename ' instead.']);
else
    input_filename=[filename '.ina'];
    output_filename=[filename '.mca'];
end


%Check validity of layers
X=size(layers);
if X(2)~=7|~isnumeric(layers)
    error('Invalid layers matrix. Must be an Xx7 numerical matrix');
end
...
%Create the input file and write all the necessary data.
[fid,message] = fopen(input_filename, 'wt');

fprintf(fid, '#File created automatically by create_FLMC_MCA_input_file...\n\n');%comment
...
%these parameters are fixed as their function is less than obvious
fprintf(fid, '\nA 1\t\t# R=Refl. T=Trans. A=Abs. dr-exit');
fprintf(fid, '\n0.0 1.0\t\t# z-source, g-source'); 

fprintf(fid, '\n\n%d\t\t\t# No. of layers',number_of_layers); % No. of layers
fprintf(fid, '\n%f\t\t# n for medium above',n_above); % n for medium above.
fprintf(fid, '\n#n mua mus g d yi muaF\t\t# One line for each layer');
for i=1:1:number_of_layers
    fprintf(fid, '\n%g %g %g %g %g %g %g\t# layer %d',layers(i,1),layers(i,2),layers(i,3),layers(i,4),layers(i,5),layers(i,6),layers(i,7),i); % layer i
end
...
end %function create_FLMC_MCA_input_file